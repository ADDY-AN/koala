...
import { Heart, Copy, Sparkles, ShieldCheck } from "lucide-react";
...
export default function DonatePage() {
    ...
    // GSAP Animations tied to the showUPI state
    useGSAP(() => {
    ...
    }, { scope: container, dependencies: [showUPI] });
                    ...
                    <CardContent className="px-8 md:px-12 pb-12">
                        {!showUPI ? (
                            <div className="space-y-10">
                                {/* Donation Amount Grid (Clean, no subtext) */}
                                <div className="grid grid-cols-2 gap-4 md:gap-6">
                                    {[
                                        ...
                                        <div
                                            ...
                                            className={`amount-btn relative cursor-pointer p-6 rounded-3xl border-2 text-center transition-colors duration-300 flex items-center justify-center min-h-[100px] ${amount === p.v ? "border-primary bg-primary/5 shadow-inner" : "border-border hover:border-primary/40 hover:bg-muted/30"}`}
                                        >
                                            {p.pop && (
                                                <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-secondary text-primary-foreground text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest shadow-md animate-bounce">
                                                    Most Popular
                                                </span>
                                            )}
                                ...
                                <div className="donate-form-area space-y-8 mt-4">
                                    <div className="space-y-3 bg-muted/20 p-6 rounded-3xl border border-border">
                                        <Label className="text-sm font-bold text-primary uppercase tracking-wider ml-2">
                                            Your Name (For the Impact Wall)
                                        </Label>
                                        <Input
                                            placeholder="e.g., Anjali from Mumbai"
                                            value={donorName}
                                            onChange={(e) => setDonorName(e.target.value)}
                                            className="h-14 rounded-2xl bg-white border-transparent shadow-sm focus-visible:ring-secondary text-lg px-6"
                                        />
                                    </div>
                                    ...
                                    <Button
                                        ...
                                        className="submit-btn w-full rounded-2xl h-16 text-xl font-bold bg-primary text-primary-foreground hover:bg-primary/90 shadow-xl shadow-primary/20 group"
                                    >
                                        <Heart className="mr-3 h-6 w-6 fill-secondary text-secondary group-hover:scale-125 transition-transform duration-300" />
                                    ...
                                    <div className="flex justify-center items-center gap-2 text-xs font-bold text-muted-foreground uppercase tracking-widest mt-2">
                                        ...
                                        <span>Secure & 80G Tax Deductible</span>
                                    </div>
                                </div>
                            </div>
                        ) : (
                            /* UPI Reveal Section with dynamic QR Code */
                            <div className="upi-reveal-container space-y-8 text-center bg-muted/10 p-2 rounded-3xl">
                                <div className="bg-white p-8 rounded-[32px] border border-border flex flex-col items-center shadow-lg relative overflow-hidden">
                                    ...
                                    <div className="upi-element qr-box bg-background p-6 rounded-3xl shadow-[0_10px_30px_rgba(0,0,0,0.08)] border border-border mb-8 flex justify-center items-center">
                                        <QRCodeSVG
                                            value={`upi://pay?pa=koalakuddle@bank&pn=Koala%20Kuddle%20Foundation&am=${amount}&cu=INR`}
                                            size={180}
                                            level={"H"}
                                            includeMargin={false}
                                            imageSettings={{
                                                src: "/images/logo.jpg", // Ensure this path matches your public folder structure
                                                x: undefined,
                                                y: undefined,
                                                height: 40,
                                                width: 40,
                                    ...
                                    <div className="upi-element bg-muted/50 p-4 w-full max-w-sm rounded-2xl border border-border flex justify-between items-center hover:bg-muted transition-colors">
                                        ...
                                        <Button
                                            variant="secondary"
                                            size="icon"
                                            className="rounded-xl h-12 w-12 hover:scale-105 transition-transform shadow-md"
                                            onClick={() => {
                                                navigator.clipboard.writeText("koalakuddle@bank");
                                                alert("UPI ID Copied!");
                                            }}
                                        >
                                ...
                                <div className="upi-element pt-4">
                                    <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest mb-4">Final Step for 80G Receipt</p>
                                    <Button
                                        className="rounded-full h-14 px-10 font-bold text-base bg-[#25D366] hover:bg-[#20b858] text-white shadow-lg shadow-[#25D366]/30 hover:-translate-y-1 transition-transform"
                                        onClick={() => window.open(`https://wa.me/919999999999?text=Hi,%20I%20just%20donated%20Rs.%20${amount}.%20Here%20is%20my%20screenshot.`, '_blank')}
                                    >
                                        WhatsApp Screenshot to Us
                                    </Button>
                                </div>
                            </div>
                        )}
